...
${CTRL_C}                 \x03
@{RESIZE_MSGS}    Resizing task completes    Resizing storage 
...
Perform Flash Image Process with Bmaptool
    ...
    #Use PSU to power off the device
    Device GPP-1326 OFF
    Sleep    1
    Device GPP-1326 ON
    ...
    ${booglog_afterresize}=      SerialLibrary.Read Until    ${LOGIN_PROMPT}
    Sleep    2
    ...
    Log    ${booglog_afterresize}
    Run Keyword And Continue On Failure    Should Contain    Should Contain Any    ${bootlog_fromuboot}    @{RESIZE_MSGS}
    Dump df output
...
Dump df output
    ...
    Sleep    5
    SerialLibrary.Write Data    ${\n}
    SerialLibrary.Write Data    df -h${\n}
    Sleep    1
    ${df_output}=    SerialLibrary.Read All Data    UTF-8
    Log    ${df_output}
...
Perform Flash Image Process with dd
    ...
    Log    ${bootlog_fromuboot}
    Log    ${booglog_afterresize}
    Run Keyword And Continue On Failure    Should Contain Any    ${bootlog_fromuboot}    @{RESIZE_MSGS}
    Dump df output
