...
${BROWSER}        Chrome
${DELAY}          0
${PORT_REGEX}      (?<=Starting server on port )(\\d+)
...
Find TCP port 
    ...
    ${URL1}=    Set Variable    http://127.0.0.1:${Find_tcp_port[0]}
    RETURN    ${URL1} 
...
Close Process 
    ${Find_tcp_port[0]}=    Find TCP port
    Close Browser
    ${result}=    Run Process    fuser -k ${Find_tcp_port[0]}/tcp    shell=True
    Log    ${result.stdout}
