...
${WIFI_MACID}=    ifconfig ${WIFI_INF} | grep -E "\w\w:\w\w:\w\w:\w\w:\w\w:\w\w"  | awk -F ' ' '{print $2}'
${WIFI_INF_N}=    ifconfig ${WIFI_INF} | grep "${WIFI_INF}" | awk -F ':' '{print $1}'
${WIFI_CHIPID}=    cat /sys/bus/mmc/devices/mmc?\:0001/mmc?\:0001\:1/device
#${WIFI_FWINFO}=    dmesg | grep "HW:" | awk -F ']' '{print $2}'
${WIFI_FWINFO}=     modinfo moal|grep -w "version" | awk -F ' ' '{print $2}'
...
${WIFI_CHIP_INFO}=    iwpriv ${WIFI_INF} get_temp
...
Board Info
   ...
   @{lines}=    Split To Lines    ${info}
    ...
    # Log    ${Board}
    ${Boardinfo1}=    Get From List    ${lines}    2
    ${Boardinfo2}=    Get From List    ${lines}    3
    ${Boardinfo3}=    Get From List    ${lines}    4
    ${Boardinfo4}=    Get From List    ${lines}    5
    ${Boardinfo5}=    Get From List    ${lines}    6
    Log To Console    \n
    Log To Console    \n================Board Info=================     
...
CPU Info
   ...
   @{lines}=    Split To Lines    ${info}
    ...
    ${CPUinfo1}=    Get From List    ${lines}    5
    ${CPUinfo2}=    Get From List    ${lines}    6
    ${CPUinfo3}=    Get From List    ${lines}    7
    ${CPUinfo4}=    Get From List    ${lines}    8
    ${CPUinfo5}=    Get From List    ${lines}    9
    ${CPUinfo6}=    Get From List    ${lines}    10
    Log To Console    \n================CPU Info=================    
...
Memory Info
   ...
   @{lines}=    Split To Lines    ${info}
    ...
    ${Meminfo1}=    Get From List    ${lines}    4
    ${Meminfo2}=    Get From List    ${lines}    5
    ${Meminfo3}=    Get From List    ${lines}    6
    ${Meminfo4}=    Get From List    ${lines}    7
    ${Meminfo5}=    Get From List    ${lines}    8
    Log To Console    \n================Memory Info================= 
...
Network Info
   ...
   Log    ${info}
   @{lines}=    Split To Lines    ${info}
   ${eth_mac}=    LAN2 Ethernet MAC Check
   ${wifi_mac}=    WIFI MAC Check
   ${bt_mac}=    BT MAC Check

    ${Netinfo1}=    Get From List    ${eth_mac}  0
    ${Netinfo2}=    Get From List    ${lines}    7
    ${Netinfo3}=    Get From List    ${lines}    9
    ${Netinfo4}=    Get From List    ${wifi_mac}  0
    ${Netinfo5}=    Get From List    ${lines}    10
    ${Netinfo6}=    Get From List    ${lines}    11
    ${Netinfo7}=    Get From List    ${lines}    12
    ${Netinfo8}=    Get From List    ${bt_mac}   0
    ${Netinfo9}=    Get From List    ${lines}    13
    ${Netinfo10}=    Get From List    ${lines}    14
    ${Netinfo11}=    Get From List    ${lines}    15
    #${wifi_ip_string}=    Strip String    @{Netinfo11}    characters=wlan0
    Log To Console    \n================Network Info================= 
    Log To Console    Ethernet MAC Address: ${Netinfo1} 
    Log To Console    Ethernet Interface: ${Netinfo2} 
    ...
    Log To Console    WIFI Driver/FW Info: ${Netinfo7} 
...
Uboot Info
   ...
   ${info}=    Seriallibrary.Read All Data        UTF-8
     Log    ${info}
   @{lines}=    Split To Lines    ${info}
   ${Ubootinfo1}=    Get From List    ${lines}    2
