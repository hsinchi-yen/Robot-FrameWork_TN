...
Check File Size
    Log To Console    file size check
    [Arguments]    ${cap_filename}
    ${file_size}=  Get File Size  ./pic_log/${cap_filename}.jpeg
    #${Chkvar}=    Run Keyword And Return Status    Should Be True    ${file_size} > ${PIC_SIZE} 
    #Log To Console     ${Chkvar}
    Should Be True    ${file_size} > ${PIC_SIZE}
...
VizionPanel Color Shift Stress Test
    ...
    FOR    ${counter}    IN RANGE    1    ${LOOP_TIME} 
        ...
        Log            <img src="pic_log/testpic_${counter}.jpeg">        html=true
        
    
        Purge capture file    testpic_${counter} 

        Check File Size        testpic_${counter}
    END
