...
Transmission Bitrate Check
    ...
    ${sender_digi}=    Get From List    ${sender_digi}    0
    ${receiver_digi}=     Get From List    ${receiver_digi}   0
    ${sender_digi} =    Evaluate    float(${sender_digi})
    ${receiver_digi} =    Evaluate    float(${receiver_digi})

    Log    sender_digi:${sender_digi} ${CRITERION_UNIT} 
    Log    receiver_digi:${receiver_digi} ${CRITERION_UNIT} 

    Should Be True        ${sender_digi} > ${100M_RATE_SPEC}
    Should Be True        ${receiver_digi} > ${100M_RATE_SPEC}
...
Bidir Transmission Bitrate Check 
    ...
    ${tx_sender_digi}=    Get From List    ${tx_sender_digi}    0
    ${tx_receiver_digi}=     Get From List    ${tx_receiver_digi}   0
    ${rx_sender_digi}=    Get From List    ${rx_sender_digi}    0
    ${rx_receiver_digi}=     Get From List    ${rx_receiver_digi}   0

    ${tx_sender_digi} =    Evaluate    float(${tx_sender_digi})
    ${tx_receiver_digi} =    Evaluate    float(${tx_receiver_digi})
    ${rx_sender_digi} =    Evaluate    float(${rx_sender_digi})
    ${rx_receiver_digi} =    Evaluate    float(${rx_receiver_digi})
    ...
    Should Be True        ${tx_sender_digi} > ${100M_RATE_SPEC}
    Should Be True        ${tx_receiver_digi} > ${100M_RATE_SPEC}
    Should Be True        ${rx_sender_digi} > ${100M_RATE_SPEC}
    Should Be True        ${rx_receiver_digi} > ${100M_RATE_SPEC}
