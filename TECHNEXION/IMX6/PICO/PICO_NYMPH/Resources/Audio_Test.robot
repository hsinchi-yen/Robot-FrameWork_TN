...
${SND_CARD_DEV}=    audiosgtl5000 
${HDMI_AUD_DEV}=    imxhdmisoc 
...
*** Keywords ***
Audio Jack Test
    ${snd_card_id}=    Sound Card ID Checker
    #play audio
...
Sound Card ID Checker
    ...
    ${cardid_fetch}=    Get Regexp Matches    ${snd_card_log}    card\\s\\d: audiosgtl5000
...
HDMI Audio ID Checker
    ...
    ${cardid_fetch}=    Get Regexp Matches    ${snd_card_log}    card\\s\\d: imxhdmisoc  
    ${cardid_1st}=    Set Variable    ${cardid_fetch}[0]
    ${return_cardid}=    Get Regexp Matches    ${cardid_1st}    ([^\\w+])\\d
